...
FG_parser = MOFFParser.from_atomistic_pdb('Nup214_CT_AF.pdb', 'Nup214_CT_CA.pdb')

# Nup214 has 9 helices, pass these as pairs
old_native_pairs = FG_parser.native_pairs.copy()
new_native_pairs = pd.DataFrame(columns=old_native_pairs.columns)
helix8=np.arange(979,985)-972
helix9=np.arange(1087,1102)-972
for i, row in old_native_pairs.iterrows():
    a1, a2 = int(row['a1']), int(row['a2'])
    if a1 > a2:
        a1, a2 = a2, a1
    flag8=( (a1 in helix8) and (a2 in helix8) )    
    flag9=( (a1 in helix9) and (a2 in helix9) )
    if flag8 or flag9:
...
box_b=100
box_c=100
insert_molecules('Nup214_CT_CA.pdb', 'start.pdb', n_mol, box=[box_a, box_b, box_c])
...
# save final system
condensate.save_system('Nup214_CT_system.xml')
